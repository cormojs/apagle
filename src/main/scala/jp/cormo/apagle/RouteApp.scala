...
import jp.cormo.apagle.endpoint.{UsersEndpoint, WebfingerEndpoint}
import jp.cormo.apagle.service.{HostMetaService, MainService}
...
object RouteApp extends App {
  ...
  val rootService = RoutingService.byMethodAndPathObject {
    ...
    case _ => (WebfingerEndpoint :+: UsersEndpoint).toService
  }
